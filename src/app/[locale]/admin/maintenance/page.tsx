...
function MaintenanceManager({ initialEnabled, initialMessage, initialUntil }: Props) {
    ...
    <div className="space-y-8">
      ...
      {enabled && (
        <div className="flex gap-3 rounded-2xl bg-amber-50 ring-1 ring-amber-100 p-4">
          ...
          <p className="text-sm text-amber-800">
            Les routes <code className="bg-amber-100 px-1 rounded">/admin</code>, <code className="bg-amber-100 px-1 rounded">/login</code> et <code className="bg-amber-100 px-1 rounded">/auth</code> restent accessibles même en mode maintenance.
          </p>
        </div>
      )}
      ...
      <div className="rounded-2xl bg-gray-50 ring-1 ring-gray-100 overflow-hidden">
        <div className="bg-gray-900 px-6 py-3">
          <p className="text-xs font-semibold uppercase tracking-widest text-gray-400">Aperçu — page publique</p>
        </div>
        <div className="p-6 flex flex-col items-center text-center gap-3">
          ...
          <p className="text-sm text-gray-600 max-w-sm">{message || <span className="text-gray-400 italic">Aucun message</span>}</p>
          {until && <p className="text-xs text-amber-700 font-medium">Retour prévu : {until}</p>}
        </div>
      </div>
    </div>
  );
...
// ── Server wrapper (loads data, renders client component) ──────────────────
import { createAdminClient } from "@/lib/supabase/admin";
import { assertAdmin } from "@/lib/supabase/assert-admin";

export default async function MaintenancePage() {
  await assertAdmin(); // guard — also called by actions, but good to have here
  ...
  return (
    <div>
      ...
      <MaintenanceManager
        ...
        initialMessage={data?.maintenance_message ?? "Le site est temporairement en maintenance. Nous serons de retour très bientôt."}
